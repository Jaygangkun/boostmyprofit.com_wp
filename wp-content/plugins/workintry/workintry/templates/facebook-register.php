<?php
/**
 * Template Name: Facebook Redirect
 *
 * @package Workintry
 * @since Workintry 1.0
 * @desc Dashboard.
...
//Facebook/Google Auth Login Library 
...
if( isset($_GET['code'] ) ){
	...
	if( !empty( $facebook_user_info['name'] ) ){
		$first_name = $facebook_user_info['name'];		
	}
	//Email
	...
	//Get usr info
	if( !empty( $first_name ) ){
		$user = get_user_by( 'login', $first_name );
	}
	if( !empty( $user->ID ) ){
    		...
    		$user_identity 	 = wp_create_user( $first_name, $random_password, $email );
		...
		<div class="container">
			<div class="row">					
				<?php 
		            ...
		            $profile_url = '';
					if( !empty($profile_page_id) ) {
						$profile_url	= codesquare_workintry_profile_menu_link($profile_page_id, 'insight', $user_identity, true);
					}
					...
					<?php if( !empty( $enable_registration ) && $enable_registration == 'enable' ){ ?>	
									...
									<form class="cp-form cp-signin-form" action="#" method="post">
										<fieldset>
											...
											<div class="form-group hp-formbtns">
												<a href="#" class="cp-btn active process-user-login"><?php esc_html_e('Join Now', 'workintry'); ?></a>	
											</div>
											<div class="form-group">
												<a href="#" data-toggle="modal" data-target="#clPwdModal"><?php esc_html_e('Forgot Password?', 'workintry'); ?>	</a>
											</div>
											<?php wp_nonce_field('login_user_request', 'login_user_request'); ?>
										</fieldset>
									</form>
									<div class="cp-signin-footer">	
										<?php if( !empty( $facebook_client_id ) && !empty( $facebook_client_secret ) ){ ?>
						...
						<div class="col-12 col-md-7 col-lg-8">						
							<div class="cp-register-banner" style="background: url(<?php echo esc_url( $lower_banner ); ?>)">
								<h2><span><?php esc_html_e('We are Open To Expand', 'workintry'); ?></span> <?php esc_html_e('Join Us To Provide Services', 'workintry'); ?></h2>
							</div>
							<form class="cp-form cp-register-form process-registration-form" action="#" method="post">
								<fieldset>
									...
									<div class="form-group cp-formbtns">
										<a href="#" class="cp-btn active process-register"><?php esc_html_e('Register Now', 'workintry'); ?></a>
										<span class="cp-checkbox">
											<input id="register" type="checkbox" name="terms">
											<?php if (!empty($terms_link)) { ?>
												<label for="register"><?php esc_html_e('Check here to indicate that you read and agree to the', 'workintry'); ?>&nbsp;<a target="_blank" href="<?php echo esc_url($terms_link); ?>"><?php esc_html_e('Terms & Conditions', 'workintry'); ?></a>&nbsp;<?php esc_html_e('and privacy policy offered by us!', 'workintry'); ?></label>
											<?php } else { ?>
						...
						<!-- Registration Ends -->	
					<?php } else { ?>									
						<div class="alert alert-danger" role="alert"><?php esc_html_e('Login/Registration disbaled by admin', 'workintry'); ?></div>
					<?php } ?>
				<?php } ?>
				<!-- </div>	 -->				
			</div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
      	<form class="cl-get-password cf-formtheme" method="post">
	        ...
	        <a type="button" class="cp-btn btn btn-default cl-get-user-password"><?php esc_html_e('Submit', 'workintry'); ?></a>
    	</form>
      </div>
